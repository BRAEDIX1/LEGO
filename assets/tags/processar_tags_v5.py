...
def ler_arquivo(caminho: Path) -> pl.DataFrame:
    ...
    # FIX: normaliza Data e Hora para string independente do dtype que o calamine retornar.
    # Data pode vir como pl.Date ou pl.Datetime; Hora pode vir como pl.Time OU pl.Datetime
    # (Excel armazena hora pura como datetime fantasma base 1899-12-31 HH:MM:SS).
    # Em todos os casos nativos, dt.strftime extrai a parte correta.
    if COL_DATA in df.columns:
        if df[COL_DATA].dtype in (pl.Date, pl.Datetime):
            df = df.with_columns(pl.col(COL_DATA).dt.strftime("%Y-%m-%d"))
        else:
            df = df.with_columns(pl.col(COL_DATA).cast(pl.Utf8, strict=False))

    if COL_HORA in df.columns:
        if df[COL_HORA].dtype in (pl.Time, pl.Datetime, pl.Date):
            # dt.strftime("%H:%M:%S") funciona para Time e Datetime (extrai só a hora)
            df = df.with_columns(pl.col(COL_HORA).dt.strftime("%H:%M:%S"))
        else:
            df = df.with_columns(pl.col(COL_HORA).cast(pl.Utf8, strict=False))
...
def _expr_data_norm() -> pl.Expr:
    # FIX: células mistas no Excel — registros antigos vêm como texto "DD/MM/YYYY"
    # e os mais recentes vêm como datetime real, que o calamine serializa para
    # "YYYY-MM-DD HH:MM:SS". O primeiro padrão (datetime-ISO) captura justamente
    # esses, que antes viravam null e eram descartados no filtro de mais recente.
    return pl.coalesce([
        pl.col(COL_DATA).str.strptime(pl.Datetime, "%Y-%m-%d %H:%M:%S", strict=False).dt.date(),
        pl.col(COL_DATA).str.strptime(pl.Date, "%Y-%m-%d", strict=False),
        pl.col(COL_DATA).str.strptime(pl.Date, "%d/%m/%Y", strict=False),
        pl.col(COL_DATA).str.strptime(pl.Date, "%d-%m-%Y", strict=False),
    ])


def _expr_hora_norm() -> pl.Expr:
    # FIX: Hora pura armazenada como datetime fantasma "1899-12-31 HH:MM:SS".
    return pl.coalesce([
        pl.col(COL_HORA).str.strptime(pl.Datetime, "%Y-%m-%d %H:%M:%S", strict=False).dt.time(),
        pl.col(COL_HORA).str.strptime(pl.Time, "%H:%M:%S", strict=False),
        pl.col(COL_HORA).str.strptime(pl.Time, "%H:%M",    strict=False),
    ])


def combiner_timestamp(df: pl.DataFrame) -> pl.DataFrame:
    # Parser tolerante com múltiplos formatos de data e hora
    return (
        df.with_columns(
            # FIX: normaliza o TAG com zeros à esquerda ANTES de deduplicar.
            # No DET o mesmo TAG aparece como "364528" e "000364528"; sem isto
            # cada forma é deduplicada em separado e colide só depois do zfill,
            # deixando linhas repetidas (algumas com o registro antigo).
            pl.col(COL_TAG).str.strip_chars().str.zfill(9).alias(COL_TAG),
            _expr_data_norm().alias("_data_norm"),
            _expr_hora_norm().alias("_hora_norm"),
        ).with_columns(
            pl.datetime(
                year   = pl.col("_data_norm").dt.year(),
                month  = pl.col("_data_norm").dt.month(),
                day    = pl.col("_data_norm").dt.day(),
                hour   = pl.col("_hora_norm").dt.hour(),
                minute = pl.col("_hora_norm").dt.minute(),
                second = pl.col("_hora_norm").dt.second(),
            ).alias("_timestamp")
        ).drop(["_data_norm", "_hora_norm"])
    )
...
def enriquecer_com_mara(df: pl.DataFrame, mara: pl.DataFrame) -> tuple:
    ...
    sem_match = df.filter(pl.col("produto").is_null()).shape[0]

    # Formata data como string DD/MM/YYYY e hora como HH:MM:SS
    df = df.with_columns(
        _expr_data_norm().dt.strftime("%d/%m/%Y").alias("data"),
        _expr_hora_norm().dt.strftime("%H:%M:%S").alias("hora"),
...
def main():
    ...
    with Progress(
        ...
        consolidado = combiner_timestamp(consolidado)
