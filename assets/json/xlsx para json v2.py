...
# Colunas obrigatórias da aba barras
BARRAS_OBRIGATORIAS = {"tag", "codigo"}

# Colunas a ignorar na aba barras (além de tag, que vira a chave)
BARRAS_IGNORAR = {"tag"}
...
def ler_barras(ws) -> tuple:
    """
    Aba 'barras': colunas tag | codigo | [colunas extras opcionais]
    Chave do dict = str(tag)

    Todas as colunas presentes além de 'tag' são incluídas no JSON.
    ...
    for row in ws.iter_rows(values_only=True):
        if cabecalho is None:
            ...
            faltando = BARRAS_OBRIGATORIAS - set(cabecalho)
            if faltando:
                print(f"ERRO: coluna(s) obrigatória(s) ausente(s) em 'barras': {faltando}")
                sys.exit(1)

            # Colunas extras = tudo que não é 'tag'
            extras = [c for c in cabecalho if c not in BARRAS_IGNORAR]
            if len(extras) > 1:
                print(f"  Colunas extras detectadas em 'barras': {[c for c in extras if c != 'codigo']}")
            continue
        ...
        # Monta entrada com todas as colunas exceto 'tag'
        entrada = {}
        for col in extras:
            val = row.get(col)
            if val is None:
                continue
            # codigo sempre como string sem decimais
            if col == "codigo":
                val = str(int(val)) if isinstance(val, float) else str(val).strip()
            else:
        ...
        if tag.isdigit() and len(tag) < TAG_DIGITS:
            suspeitos.append((tag, entrada.get("codigo", "")))
