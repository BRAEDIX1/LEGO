...
def montar_arvore():
    ...
    for colecao, itens in data.items():
        # FIX: ignora chaves que não são dicionários (ex: "version")
...
def mostrar_detalhes(event):
    ...
    if colecao in data and isinstance(data[colecao], dict) and codigo in data[colecao]:
...
def buscar():
    ...
    if not codigo:
        resultado.insert(tk.END, "Digite um código para buscar.")
        return
    ...
    # FIX: feedback quando não encontra nada
    if not encontrou:
        resultado.insert(tk.END, f"Código '{codigo}' não encontrado em nenhuma coleção.")
...
def relacoes():
    ...
    # FIX: guarda contra coleções que não são dict
    if colecao not in data or not isinstance(data[colecao], dict):
        messagebox.showwarning("Aviso", "Coleção inválida para análise de relações.")
        return
    ...
    if colecao == "gases":
        ...
        if "barras" in data and isinstance(data["barras"], dict):
    ...
    # ----------------------------
        ...
        if isinstance(info, dict) and "codigo" in info:
            ...
            if "gases" in data and isinstance(data["gases"], dict) and cod_gas in data["gases"]:
...
def exportar():
    ...
    try:
        # FIX: import lazy — só carrega pandas quando realmente necessário
        import pandas as pd
